...
class App():
    ...
    def __reset_screen(self):
        ...
        self.__search_graph_label.blit(text_element,
                                       (GRAPH_WIDTH // 2 - text_element.get_width() // 2, 1))
    ...
    def __draw_graphs(self) -> None:
        ...
        for i, (graph, graph_surface) in enumerate(zip(self.__graphs,
                                                       self.__graph_surfaces),
                                                   start=1):
    ...
    def __draw_vertices(self, graph: Graph, graph_surface: pg.surface.Surface) -> None:
        ...
        for node_key in graph.vertices:
            ...
            for neighbour_key in neighbours:
                ...
                self.__draw_vertex(node, neighbour, weight,
                                   VERTEX_COLOR, VERTEX_WIDTH, graph_surface)
    ...
    def __draw_optimal_path(self, graph: Graph, graph_surface: pg.surface.Surface) -> None:
        ...
        for node_key, neighbour_key, weight in graph.optimal_cycle.vertices:
            ...
            neighbour = graph[neighbour_key]
            self.__draw_vertex(node, neighbour, weight,
                               BEST_PATH_VERTEX_COLOR, BEST_PATH_VERTEX_WIDTH, graph_surface)
    ...
    def __draw_vertex(self, node: Node, neighbour: Node, weight: int,
        ...
        pg.draw.line(graph_surface, vertex_color, node.position,
                    neighbour.position, vertex_width)
    ...
    def __calculate_distances(self, nodes: list[Node]) -> None:
        ...
        for node in nodes:
            for other in nodes:
                if (node != other and
                    (node.key, other.key) not in self.__distances and
                    (other.key, node.key) not in self.__distances):
    ...
    def __create_results_display(self, results: list[AlgorithmResult]) -> pg.Surface:
        ...
        text = (f"Results for {node_count} nodes " +
                f"({possible_paths} possible paths):")
            ...
            match event.type:
                    ...
                    match event.key:
                        ...
                        case pg.K_RETURN | pg.K_KP_ENTER:
                            if not self.__running:
                                ...
                                if len(nodes) <= 2:
                                    continue
                                self.__graphs = (Graph(nodes.copy()), Graph(nodes.copy()),
                                                Graph(nodes.copy()), Graph(nodes.copy()))
                ...
                case pg.MOUSEBUTTONDOWN:
                    ...
                    if mouse_button[0]:
                        ...
                        if (((WIDTH//2) - (GRAPH_WIDTH//2)) < mouse_x
                                                            < ((WIDTH//2) + (GRAPH_WIDTH//2)) and
                            (MARGIN + TEXT_SIZE) < mouse_y < (MARGIN + TEXT_SIZE + GRAPH_HEIGHT)):
                            node_x, node_y = (mouse_x - ((WIDTH//2) - (GRAPH_WIDTH//2)),
                                              mouse_y - TEXT_SIZE)
